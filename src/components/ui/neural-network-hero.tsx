...
export default function Hero({
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-full overflow-hidden">
      ...
      <div className="relative mx-auto flex h-full max-w-7xl flex-col items-start justify-center gap-6 px-6 pb-24 pt-36 sm:gap-8 md:px-10 lg:px-16">
        ...
        <h1 ref={headerRef} className="max-w-2xl text-left font-heading text-5xl font-bold leading-[1.05] tracking-[-0.02em] text-white sm:text-6xl md:text-7xl">
          {title}
        </h1>
        ...
        <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2">
          {ctaButtons.map((button, index) => (
            <a
              key={index}
              href={button.href}
              className={`rounded-sm border px-7 py-3.5 text-[13px] font-medium tracking-[0.15em] uppercase transition-all duration-500 focus:outline-none focus:ring-2 focus:ring-white/30 ${
                button.primary
                  ? "border-champagne/30 bg-primary text-champagne hover:border-champagne hover:bg-champagne hover:text-primary hover:shadow-lg"
                  : "border-neutral-200 text-neutral-200 hover:border-magenta hover:text-champagne"
              }`}
            >
              {button.text}
            </a>
        ...
        <ul ref={microRef} className="mt-4 flex flex-wrap gap-6 text-xs font-extralight tracking-tight text-white/50">
          {microDetails.map((detail, index) => {
            const refMap = [microItem1Ref, microItem2Ref, microItem3Ref];
            return (
              <li key={index} ref={refMap[index]} className="flex items-center gap-2">
                <span className="h-1 w-1 rounded-full bg-champagne/40" /> {detail}
              </li>
            );
          })}
        </ul>
